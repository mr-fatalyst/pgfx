...
def render():
    ...
    pgfx.text(font, " + ".join(held) if held else "(none)", 20, 60,
              pgfx.GREEN if held else DIM)
    ...
    buttons = [
        label
        for label, btn in (("Left", pgfx.MOUSE_LEFT), ("Right", pgfx.MOUSE_RIGHT),
                           ("Middle", pgfx.MOUSE_MIDDLE))
        if pgfx.mouse_down(btn)
    ]
    pgfx.text(font, f"mouse: {mx}, {my}", SCREEN_W - 40, 30, pgfx.WHITE, align="right")
    pgfx.text(font, " + ".join(buttons) if buttons else "(no buttons)", SCREEN_W - 40, 60,
              pgfx.GREEN if buttons else DIM, align="right")
    ...
    pgfx.text(font, "PRESS ANYTHING — ESC TO QUIT", SCREEN_W / 2, SCREEN_H - 34, DIM,
              align="center")
